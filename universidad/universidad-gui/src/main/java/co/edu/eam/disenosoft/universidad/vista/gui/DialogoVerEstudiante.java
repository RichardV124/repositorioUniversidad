...
public class DialogoVerEstudiante extends javax.swing.JDialog {
     ...
     * Creates new form DialogoVerEstudiante
     */
    public DialogoVerEstudiante(RegistroCurso registro) {
        super();
        this.registro = registro;
        controlador = new ControladorDialogoVerEstudiante();
        initComponents();
        llenarDatos(registro);
        //Dejar esta linea de ultimo siempre!!!!!
        setVisible(true);
    }

    
    private void llenarDatos(RegistroCurso registro) {
        try {
            tfcedulaestudiante.setText(registro.getEstudiante().getCedula());
            tfnombreestudiante.setText(registro.getEstudiante().getNombre() + " "
                    + registro.getEstudiante().getApellido());
            int creditos = controlador.contarCreditosEstudiante(registro.getEstudiante());
            tfcreditosregistradosestudiante.setText("" + creditos);            
            List<Evaluacion> evaluaciones = controlador.listarEvaluacionesPorCurso(registro.getCurso());
            llenarTabla(evaluaciones);
            lblDefinitiva.setText(""+calcularDefinitiva(evaluaciones));
        } catch (Exception ex) {
    ...
    private void llenarTabla(List<Evaluacion> evaluaciones) throws Exception {
        DefaultTableModel tabla = (DefaultTableModel) tablaNotas.getModel();
        tabla.setRowCount(0);
        for (Evaluacion eval : evaluaciones) {
            String nombre = eval.getNombre();
            double porcentaje = eval.getPorcentaje();
            Nota nota = controlador.buscarNotaPorEvaluacion(eval);
            String valorNota = "NC";
            if (nota != null) {
                valorNota = ((Double) nota.getValor()).toString();
            }
            tabla.addRow(new Object[]{nombre, valorNota, porcentaje});
        }
    }
    
    private double calcularDefinitiva(List<Evaluacion> evaluaciones) throws Exception {
        double definitiva = 0;
        for (Evaluacion eval : evaluaciones) {
            Nota nota = controlador.buscarNotaPorEvaluacion(eval);
            if (nota != null) {
                definitiva = nota.getValor()*(eval.getPorcentaje()/100);
            }
        }
        return definitiva;
    }
